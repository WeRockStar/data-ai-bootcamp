...
with DAG(
    ...
    def generate_crypto_flex(data):
        ...
        return FlexMessage(
            alt_text="Cryptocurrency Prices",
            contents=FlexContainer.from_dict(flex_bubles),
        )

    def send_line_notify(**kwargs):
        access_token = Variable.get("CHANNEL_ACCESS_TOKEN")
        user_id = Variable.get("LINE_USER_ID")
        crypto_data = kwargs["ti"].xcom_pull(
            key="data", task_ids="collect_coingecko_data_task"
        )
